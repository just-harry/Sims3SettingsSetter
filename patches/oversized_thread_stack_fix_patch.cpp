...
class OversizedThreadStackFix : public OptimizationPatch {
  ...
  public:
    ...
    void RenderCustomUI() override {
        ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "Address-space saved: %.3f MB", static_cast<float>(addressSpaceSaved.load(std::memory_order_relaxed)) / 1048576.0f);
...
REGISTER_PATCH(OversizedThreadStackFix,
    {.displayName = "Oversized Thread Stack Fix",
        .description = "Reduces the memory used by background threads that the game creates a few-dozen of for file IO.",

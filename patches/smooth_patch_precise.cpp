...
uint64_t WaitUntilPrecisely(uint64_t time, uint64_t now) {
    ...
    if (sleepUntil > now) {
        ...
        // A relative sleep instead of an absolute sleep.
...
class SmoothPatchPrecise : public OptimizationPatch {
  ...
  public:
    ...
    bool UpdateTimerFrequency() {
        ...
        ULONG oldTimerResolution = timerResolution;
        NtQueryTimerResolution(&maxTimerResolution, &minTimerResolution, &timerResolution);
        if (timerResolution != oldTimerResolution) {
